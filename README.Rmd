...
```{r results='hide', eval=T,echo=F, message = F, warning = F}
...
```{r carto_com_full, eval=T, echo=F, message=F, warning=F , dpi= 500}
...
DEP_FRMETDROMCOM.proche <-
  transfo_om(shape_origine = DEP_FRMETDROM %>%
               ...
               rbind.data.frame(DEP_WF %>% rename(insee_dep = INSEE_DEP)) %>%
               rbind.data.frame(DEP_PO %>% rename(insee_dep = INSEE_DEP)) %>%
               rbind.data.frame(DEP_NC %>% rename(insee_dep = INSEE_DEP)),
             var_departement = "insee_dep",
             type_transfo = "v2")
